      ...
      for (int head_idx = 0; head_idx < num_heads; ++head_idx) {
        ...
        if (alibi_slopes != 0) {
          ...
          for (int token_idx = 0; token_idx < context_len; ++token_idx)
            head_logit_ptr[token_idx] += alibi_slope * (token_idx - context_len + 1);
        }
...
void paged_attention_v1_impl_launcher(
    ...
    ov::Tensor alibi_slopes, int max_context_len) {
  ...
  T *alibi_slopes_ptr = alibi_slopes.get_shape()[0] > 0 ? alibi_slopes.data<float>() : nullptr;
...
#define CALL_KERNEL_LAUNCHER(T, BLOCK_SIZE)                                    \
  paged_attention_v1_impl_launcher<T, BLOCK_SIZE>(                             \
      ...
      context_lens, alibi_slopes, max_context_len);
...
void paged_attention_v1_cpu(ov::Tensor out, ov::Tensor query,
                            ...
                            ov::Tensor context_lens, ov::Tensor alibi_slopes,
